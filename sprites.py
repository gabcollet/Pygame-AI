...
# same as collide with walls but using hit_rect instead. 
# I should make a more versatile function
def collide_with_npc(sprite, group, dir):
	if dir == 'x':
		hits = pg.sprite.spritecollide(sprite, group, False, collide_hit_rect)
		if hits:
...
class Player(pg.sprite.Sprite):
	...
	#collide with walls and npc
		self.hit_rect.x = self.pos.x
		collide_with_npc(self, self.game.npc, 'x')
		collide_with_walls(self, self.game.walls, 'x')
		self.hit_rect.y = self.pos.y
		collide_with_npc(self, self.game.npc, 'y')
...
class Tree(pg.sprite.Sprite):
	#Note: I'm using a wall tile on the center of the tree to make collision
...
class NPC(pg.sprite.Sprite):
	#Note: Like for tree i'm using wall for collision. I know that is bad...
	def __init__(self, game, x, y):
		self._layer = MOB_LAYER
		self.groups = game.all_sprites, game.npc
		pg.sprite.Sprite.__init__(self, self.groups)
		self.game = game
		self.image = game.npc_img.copy()
		self.rect = self.image.get_rect()
		self.rect.center = (x, y)
		self.rect.move_ip(10, -14)
