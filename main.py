...
class Game:
	...
	#Set folder and font
		...
		self.title_font = ('img/Font/RioGrande.ttf')
		self.hud_font = ('img/Font/Cowboys.ttf')
		self.text_font = ('img/Font/OldTownRegular.ttf')
	...
	#set sprites images
		...
		self.npc_img = pg.transform.scale(self.npc_img, (54, 54))
	...
	def	new(self):
	#Initialize new assets and variable in game
	...
	def run(self):
		...
		while self.playing:
			...
			self.events()
			if not self.paused and not self.chat:
				self.update()
			self.draw()
	...
	#Player dont get over npc
		for npc in self.npc:
			if self.player.hit_rect.y < npc.hit_rect.y:
					self.all_sprites.change_layer(npc, 4)
			else:
				self.all_sprites.change_layer(npc, 2) 
	#HUD functions
		...
		if self.paused:
			...
			self.draw_text("(Paused)", self.title_font, 105, RED, WIDTH / 2, HEIGHT / 2 - 50, align="center")
			self.draw_text("Press ENTER to quit or ESC to continue", self.title_font, 50, WHITE, WIDTH / 2, HEIGHT * 6 / 10 - 50, align="center")
		if self.chat:
			# chat_screen(self)
			Chat(self).chat_screen(self)
		pg.display.flip()
	...
	#Map Events
		for event in pg.event.get():
			...
			if event.type == pg.KEYUP:
				...
				if event.key == pg.K_e:
					for npc in self.npc:
						if pg.sprite.collide_rect(self.player, npc):
	...
	def show_start_screen(self):
		...
		self.draw_text("Pause / Quit ---------------------------- ESC", self.text_font, 25, BLACK, 20, HEIGHT -110, align="nw")
		self.draw_text("Talk to NPC -------------------------------- E", self.text_font, 25, BLACK, 20, HEIGHT -90, align="nw")
		self.draw_text("Debug mode -------------------------------- H", self.text_font, 25, BLACK, 20, HEIGHT -70, align="nw")
		pg.display.flip()
		self.wait_for_key()
